...
public class XLineLayout extends XViewGroup {
    /**
     * 两个View之间距的比例
     */
    ...
    public XLineLayout(Context context) {
        this(context, null);
    }

    public XLineLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XLineLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.XLineLayout);
        // 默认是在一半的位置
        mViewMarginRate = array.getFloat(R.styleable.XLineLayout_lineViewMarginRate, 0.5f);
        // 默认第一个在上面
        mIsReverse = array.getBoolean(R.styleable.XLineLayout_lineIsReverse, mIsReverse);
        array.recycle();
        // 设置充许改变绘制顺序
